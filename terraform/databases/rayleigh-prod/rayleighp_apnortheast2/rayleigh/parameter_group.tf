# Aurora Parameter Group in RDS
# This is for the database instance not the cluster.
resource "aws_db_parameter_group" "rayleigh_aurora_pg" {
  name   = "rayleigh-aurora-${data.terraform_remote_state.vpc.outputs.shard_id}-pg"
...
resource "aws_rds_cluster_parameter_group" "rayleigh_aurora_cluster_pg" {
  name        = "rayleigh-aurora-${data.terraform_remote_state.vpc.outputs.shard_id}-cluster-pg"

  # Please change this value to version you want to use 
  family      = "aurora-mysql5.7"

  description = "rayleigh RDS cluster parameter group"
